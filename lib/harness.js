...
(function (global) {
  ...
  // A case whose subject is the document's own URL declares one:
  //
  //     url: "http://oracle.test/page/index.html"
  //
  // Both runners then load the case's markup AT that URL, so `location`, the
  // base URL a relative href resolves against, and `document.baseURI` mean the
  // same thing on both sides. Without it each side has whatever its runner
  // happened to give a blank document, and nothing about URLs is comparable.
  // The host is fictional: the browser side answers the request itself rather
  // than reaching the network, and Dommy fetches nothing.
  global.__oracleUrl = function () {
    return cases.length === 1 && cases[0].url ? cases[0].url : null;
  };

  // A case that would otherwise be one very long run can declare that it is
  // divisible:
  //
  //     shard: { count: "seeds", from: "seedFrom", to: "seedTo", size: 25 }
  //
  // — the `seeds` config is a count the runner may cut into slices of 25,
  // handing each slice its bounds through the `seedFrom` / `seedTo` config keys
  // the case reads, running each in a FRESH engine and merging the returned
  // objects. A case that shards must therefore return an object keyed per unit,
  // which is what makes the merge meaningful.
  //
  // The point is the fresh engine. Cost that accumulates across units —
  // per-node host proxies on the QuickJS side above all — is what put a ceiling
  // on the fuzzer: at 150 seeds x 250 steps the VM passed its 512MB limit and
  // QuickJS surfaced it as a bare thrown null. Sliced, each run starts clean and
  // the budget stops being bounded by one engine's lifetime.
  global.__oracleShard = function () {
    return cases.length === 1 && cases[0].shard ? cases[0].shard : null;
  };
